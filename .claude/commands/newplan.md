...
**IMPORTANT: This command is user-initiated only. Never call `/newplan` automatically.**
...
Follow every step of the `clearplan` command exactly:
...
Follow every step of the `continueplan` command exactly:
...
4. When the TODO is done, mark it `[x]` in the phase file and update `plan/state.md` (current TODO advances to the next item, last updated date updated, progress counts updated).
...
6. If a phase is fully complete, mark it `✅ Done` in `plan/index.md` and update `plan/state.md` to point to the next phase.

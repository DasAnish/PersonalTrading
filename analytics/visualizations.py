...
from .summary import calculate_return_attribution
...
def plot_portfolio_comparison(
    ...
    ax1.set_title(
        "Portfolio Value Over Time (with Individual ETF Benchmarks)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for i, (name, results) in enumerate(results_dict.items()):
        ...
        ax2.plot(history.index, drawdown, color=colors[i % len(colors)], linewidth=1.5)
    ...
    if not first_results.weights_history.empty:
        ...
        ax3.set_title(
            f"Portfolio Weights Over Time - {first_name}",
            fontsize=14,
            fontweight="bold",
        )
    ...
    if not attribution.empty and len(attribution.columns) > 0:
        ...
        ax4.text(
            0.5,
            0.5,
            "No return attribution data available",
            ha="center",
            va="center",
...
def plot_transaction_analysis(
    results: BacktestResults, figsize: tuple = (12, 6), save_path: str = None
) -> plt.Figure:
    """
    ...
    fig.suptitle(
        f"Transaction Analysis - {results.strategy_name}",
        fontsize=14,
        fontweight="bold",
    )

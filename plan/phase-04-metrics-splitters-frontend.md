...
## TODOs
- [x] Delete `_compute_cagr` / `_compute_omega_ratio` from `scripts/server/api.py:14–43` — they duplicate `analytics/metrics.py` `calculate_cagr` (:186) and `calculate_omega_ratio` (:229); call the analytics functions from the endpoints instead
- [x] Add `history_to_series(portfolio_history: list) -> pd.Series` helper in `scripts/server/data.py`; replace the 5 copy-pasted list→Series conversions in api.py (lines ~138, 169, 306, 375, 381)
- [x] Split `analytics/metrics.py` (684 lines): move lines 521–684 (`calculate_monthly_returns`, `calculate_rolling_metric`, `generate_metrics_summary`, `calculate_return_attribution`) to new `analytics/summary.py`; re-export via `analytics/__init__.py` so existing importers (run_backtest.py:39, scripts/test_hrp_backtest.py:21) keep working
...
- [x] Splitter unit tests in `tests/test_optimization.py` (or new file): exact fold boundaries, residual-discard behavior preserved vs old implementations, embargo/purge row exclusion
